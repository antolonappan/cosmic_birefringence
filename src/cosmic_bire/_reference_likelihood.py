...
class cosmic_birefringence:
	...
	def __init__(self, parameters):
		...
		if self.params.get('computation_backend', 'python') == 'fortran':
			from . import tools_fortran
			self.tools = tools_fortran
		else:
			self.tools = tools_fast
		nob = self.params['nob']
		...
		cov_filename = self.params['covariance_file']
		calculate_cov = os.path.isfile(cov_filename) == False
		if calculate_cov:
			# Here we calculate the covariance matrix. With many bands, this will take some time
			start = time.time()
			print('Calculating covariance matrix')
			self.obs_cov = self.tools.get_covariance(size, l_max, nob, self.observed_cross_spectra, self.params['f'])
		...
		# Avoid repeated F2PY copies during MCMC. This changes storage order only;
		# values and the likelihood calculation are unchanged.
		if self.params.get('computation_backend', 'python') == 'fortran':
			self.c_l_o_bin = np.asfortranarray(self.c_l_o_bin)
			self.c_l_th_bin = np.asfortranarray(self.c_l_th_bin)
			self.obs_cov_bin = np.asfortranarray(self.obs_cov_bin)

	def import_cls(self):
		# Load the observed power spectra
		cl_filename = self.params['spectra_file']
		...
		for l_0 in range(l_bin):
			...
			for i in range(nob):
				for j in range(nob):
					for im in range(2):
						for jm in range(2):
							if self.tools.do_not_count_this_eq(i, j, im, jm):
	...
	def log_prob(self, theta, beta_i='none'):
		...
		if self.params['psi_l'] == '':
			# No EB modeling
			prob = self.tools.likelihood_prob(self.c_l_o_bin, self.c_l_th_bin, self.obs_cov_bin, self.params['with_ln_det_cov'], bin_range=self.number_of_bins, nob=nob, alpha=correct_format_alpha, beta=beta)
		else:
			# We model the dust EB
			prob = self.tools.likelihood_prob_model_eb(self.c_l_o_bin, self.c_l_th_bin, self.obs_cov_bin, self.params['with_ln_det_cov'], bin_range=self.number_of_bins, nob=nob, alpha=correct_format_alpha, beta=beta, psi_l=self.psi_l, A=A, turn_off_for_lowest_indices = self.params['turn_off_for_lowest_indices'])
	...
	def run_sampler(self, it=500, burnin=100, fig_name='default', no_beta = False):
		...
		os.makedirs('img/{}'.format(params['data_set']), exist_ok=True)
	...
	def plot_eb_mean(self, beta, alpha, A, best_fit_params):
		...
		eb_mean, eb_std, chi_squared_all, chi_squared = self.tools.get_inverse_variance_mean_std(
			self.c_l_o_bin,
			self.c_l_th_bin,
			self.obs_cov_bin,
		...
		for i in tqdm.tqdm(range(n)):
			eb_alpha[i, :], eb_beta[i, :], _, _  = self.tools.get_inverse_variance_mean_std(
				self.c_l_o_bin,
				self.c_l_th_bin,
				self.obs_cov_bin,
				bin_range=self.number_of_bins,
		...
		rms_mean, rms_std, chi_squared_all, chi_squared_stacked = self.tools.get_inverse_variance_mean_std(
			self.c_l_o_bin,
			self.c_l_th_bin,
			self.obs_cov_bin,
			bin_range=self.number_of_bins,
